...
public class ThreadSafeEventService implements EventService {
   ...
   * or use getVetoEventListeners to get topic and pattern matchers. In EventBus
   * 2.0 this name will replace getVetoEventListeners() and have it's union
   * functionality
   */
...
//         }
   ...
   * toRemove element from the List in the map for the given key. The entire map
   * is checked for WeakReferences and ProxySubscribers and they are all
   ...
   * This overrides any setting for the DefaultCacheSizePerClassOrTopic.
   * <p/>
   * Settings for exact topic names take precedence over pattern matching. If a
   * topic matches the cache settings for more than one pattern, the cache size
   * chosen is an undetermined one from one of the matched pattern settings.
   * <p/>
   * The cache for a topic is not adjusted until the next publication on that
   * topic.
   *
   ...
   * #setCacheSizeForTopic(String, int) and #setCacheSizeForTopic(Pattern, int).
   *
   ...
   * @return the real value of the param, or the proxied subscriber of the param
   * if the param is a a ProxySubscriber
   */
      ...
      if (cleanupTimer == null) {
        cleanupTimer = Executors.newSingleThreadScheduledExecutor(new ThreadFactory() {
          ...
          public Thread newThread(Runnable r) {
            Thread t = new Thread(r, "Nifty Event Service Cleanup #" + threadIndex.incrementAndGet());
            t.setDaemon(true);
