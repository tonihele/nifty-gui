 ...
 * events or topic data, call
 * {@link #setDefaultCacheSizePerClassOrTopic(int)}, {@link #setCacheSizeForEventClass(Class, int)},
 * or
 * {@link #setCacheSizeForTopic(String, int)}, {@link #setCacheSizeForTopic(Pattern, int)}.
 * Retrieve cached values with
 * {@link #getLastEvent(Class)}, {@link #getLastTopicData(String)}, {@link #getCachedEvents(Class)},
 * or {@link #getCachedTopicData(String)}. Using caching while subscribing is
 * most likely to make sense only if you subscribe and publish on the same
 * thread (so caching is very useful for Swing applications since both happen on
 * the EDT in a single-threaded manner). In multithreaded applications, you
 * never know if your subscriber has handled an event while it was being
 * subscribed (before the subscribe() method returned) that is newer or older
 * than the retrieved cached value (taken before or after subscribe()
 * respectively).
 * <p/>
 ...
 * SubscriberTimingEvents, see the constructor
 * {@link ThreadSafeEventService (long, boolean)}. The timing is checked for
 * veto subscribers too.
 * <p/>
 * <h2>Logging</h2>
 * <p/>
 ...
 * StackTraceElement[],String)} to handleException exceptions in another way.
 * Each call to a subscriber is wrapped in a try block to ensure one listener
 ...
 * the
 * <tt>CLEANUP_PERIOD_MS_DEFAULT<tt> (20 minutes) by default. The default is
 * overridable in the constructor or via #setCleanupPeriodMS(Integer). If set to
 * null, cleanup will not start. This is implemented with a
 * <tt>java.util.Timer</tt>, so Timer's warnings apply - setting this too low
 ...
 * or below the <tt>cleanupStopThreshhold</tt>
 * cleanup stops until the <tt>cleanupStartThreshhold</tt> is exceeded again.
 * The <tt>cleanupStopThreshhold</tt> is set to
...
public class ThreadSafeEventService implements EventService {
    ...
    private Runnable cleanupTimerTask;
    ...
    /**
     * Creates a ThreadSafeEventService that does not monitor timing of
     * handlers.
     */
     ...
     * subscriber should spend handling an event, The service will publish an
     * SubscriberTimingEvent after listener processing if the time was exceeded.
     ...
     * SubscriberTimingEvent after listener processing if the time was exceeded.
     * If null, no EventSubscriberTimingEvent will be issued.
     * @param subscribeTimingEventsInternally add a subscriber to the
     * SubscriberTimingEvent internally and call the protected subscribeTiming()
     * method when they occur. This logs a warning to the {@link Logger} by
     * default.
     */
     ...
     * timeThresholdForEventTimingEventPublication is null and
     ...
     * cleanup.
     *
     * @param cleanupStopThreshold threshold at which cleanup stops (it may
     * start again).
     */
     ...
     * @param vetoListenerMap the internal map of veto listeners to use (by
     * topic of class)
     ...
     * class)
     * @param subscriber the EventSubscriber or EventTopicSubscriber to
     * subscribe, or a WeakReference to either
     *
     ...
     * @param subscriber the subscriber to unsubscribe, either an
     * EventSubscriber or an EventTopicSubscriber, or a WeakReference to either
     *
     ...
     *
     * @param o the topic or event class to unsubscribe from
     * @param vetoListenerMap the map of veto listeners to use (by topic or
     * class)
     ...
     * null
     * @param subscribers the subscribers to publish to - must be a snapshot
     * copy
     ...
     * event is not null
     * @param eventObj the payload being published on the topic , will be null
     ...
     * @return the same list if there are no prioritized subscribers in the
     * list, otherwise a new sorted result
     */
     ...
     * non-null
     * @param eventObj the eventObj about to be published on a topic, null if
     * the event is non-null
     */
     ...
     * or use getVetoEventListeners to get topic and pattern matchers. In
     * EventBus 2.0 this name will replace getVetoEventListeners() and have it's
     * union functionality
     */
...
//         }
    ...
    /**
     * Given a Map (of Lists of subscribers or veto listeners), removes the
     * toRemove element from the List in the map for the given key. The entire
     * map is checked for WeakReferences and ProxySubscribers and they are all
     * unsubscribed if stale.
     *
     * @param map map of lists
     * @param key key for a List in the map
     * @param toRemove the object to remove form the list with the key of the
     * map
     *
     ...
     * resolving WeakReferences to hard references, and removing garbage
     * collected references from the original set.
     *
     ...
     * caching).
     * <p/>
     * If this value is set to a positive number, then when an event is
     * published, the EventService caches the event or topic payload data for
     * later retrieval. This allows subscribers to find out what has most
     * recently happened before they subscribed. The cached event(s) are
     * returned from #getLastEvent(Class), #getLastTopicData(String),
    ...
    /**
     * @return the default number of event payloads kept per event class or
     * topic
     */
     ...
     * Class.getInterfaces(), if multiple cache sizes are set, the interface
     * names declared earliest in the implements clause of the eventClass takes
     * effect.
     * <p/>
     ...
     * topic. By default, no caching is done.
     * <p/>
     * This overrides any setting for the DefaultCacheSizePerClassOrTopic.
     * <p/>
    ...
    /**
     * Set the number of published data objects cached for topics matching a
     * pattern. By default, caching is done.
     * <p/>
     * This overrides any setting for the DefaultCacheSizePerClassOrTopic.
     * <p/>
     * Settings for exact topic names take precedence over pattern matching. If
     * a topic matches the cache settings for more than one pattern, the cache
     * size chosen is an undetermined one from one of the matched pattern
     * settings.
     * <p/>
     * The cache for a topic is not adjusted until the next publication on that
     * topic.
    ...
    /**
     * Returns the number of cached data objects published on a particular
     * topic. By default, no caching is performed.
     * <p/>
     * This result is computed for a particular topic from the values passed to
     * #setCacheSizeForTopic(String, int) and #setCacheSizeForTopic(Pattern,
     * int).
     *
     ...
     * @param eventClass an index into the cache, cannot be an interface
     *
     * @return the last events published for this event class, or null if
     * caching is turned off (the default)
     */
     ...
     *
     * @return the last data Object published on this topic, or null if caching
     * is turned off (the default)
     */
    @Override
    ...
    /**
     * Clears the event cache for a specific event class or interface and it's
     * any of it's subclasses or implementing classes.
     *
    ...
    /**
     * Clears the topic data cache for all topics that match a particular
     * pattern.
     *
    ...
    /**
     * All exception handling goes through this method. Logs a warning by
     * default.
     */
    ...
    /**
     * Unsubscribe a subscriber if it is a stale ProxySubscriber. Used during
     * subscribe() and in the cleanup Timer. See the class javadoc.
     * <p/>
     * Not private since I don't claim I'm smart enough to anticipate all needs,
     * but I am smart enough to doc the rules you must follow to override this
     * method. Those rules may change (changes will be doc'ed), override at your
     * own risk.
     * <p/>
     * Overriders MUST call iterator.remove() to unsubscribe the proxy if the
     * subscriber is a ProxySubscriber and is stale and should be cleaned up. If
     * the ProxySubscriber is unsubscribed, then implementers MUST also call
     * proxyUnsubscribed() on the subscriber. Overriders MUST also remove the
     * proxy from the weakProxySubscriber list by calling
     * removeStaleProxyFromList. Method assumes caller is holding the
     * listenerList lock (else how can you pass the iterator?).
     *
     * @param iterator current iterator
     * @param existingSubscriber the current value of the iterator
     * @return the real value of the param, or the proxied subscriber of the
     * param if the param is a a ProxySubscriber
     */
    ...
    /**
     * Increment the count of stale proxies and start a cleanup task if
     * necessary
     */
    ...
    private void startCleanup() {
        synchronized (listenerLock) {
            if (cleanupTimer == null) {
                cleanupTimer = Executors.newSingleThreadScheduledExecutor(new ThreadFactory() {

                    private final AtomicInteger threadIndex = new AtomicInteger(0);

                    @Override
                    public Thread newThread(Runnable r) {
                        Thread t = new Thread(r, "NiftyEventServiceCleanup_" + threadIndex.incrementAndGet());
                        t.setDaemon(true);
            ...
            if (cleanupTimerTask == null) {
                ...
                cleanupTimer.scheduleWithFixedDelay(cleanupTimerTask, 0L, cleanupPeriodMS, TimeUnit.MILLISECONDS);
            }
        }
    }
